...
const createCourseValidation = [
  ...
  body('code')
    .trim()
    .custom(customValidators.isValidCourseCode)
    .withMessage('Course code must be in valid format (e.g., CS101)'),
  body('instructor')
    .trim()
    .isLength({ min: 2, max: 100 })
  ...
  body('color')
    .optional()
    .custom(customValidators.isValidColor)
...
router.post('/', sanitizeInput, createCourseValidation, validate, createCourse);
router.get('/stats', getCourseStats);
router.get('/:id', getCourse);
router.put('/:id', sanitizeInput, updateCourseValidation, validate, updateCourse);
